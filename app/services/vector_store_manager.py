...
class VectorStoreManager:
    ...
    def __init__(self):
        ...
        # 不在这里连 Milvus：这个模块在 import 链上（knowledge_tool -> tools -> agent），
        # 在构造函数里连意味着 Milvus 没起来时，整个进程连 import 都过不去——包括根本不做
        # 检索的 Slack bot 和测试。改成第一次真正用到时再连。
...
def get_vector_store_manager() -> "VectorStoreManager":
    ...
    if _manager is None:
        ...
        _manager._initialize_vector_store()
    return _manager


class _LazyVectorStoreManager:
    """保持既有 `vector_store_manager.xxx` 调用点不变，把连接推迟到第一次访问。"""
